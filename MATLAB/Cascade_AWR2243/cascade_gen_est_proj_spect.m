...
outpath = '/mnt/HDD04/Projection_data/OUTPUTS/Cascade_AWR2243/est_projected_microDoppler/';
...
for i = 1:length(subfolds) % 234, 239, 224 worked
        tic
        ...
        if isstring(proj_angles)
                continue
        end
        slash_id = find(fOut == '/');
        disp([fOut(slash_id(end)+1:end-4) ' --> Estimated angles: ' int2str(proj_angles)]);
        RDC_to_microDoppler_projection_fun(RDC, NPpF, proj_angles, fOut)
        toc
