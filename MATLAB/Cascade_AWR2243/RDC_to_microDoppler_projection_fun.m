function [] = RDC_to_microDoppler_projection_fun(RDC, NPpF, ang, fNameOut)
        SweepTime = 40e-3;
...
%         for i = 1:length(ang)%9 % divide into 10 degrees, set limit to 9
            ...
            for a = 1:length(ang)
                  ...
                  RDCs{a} = zeros(size(RDC,1), size(RDC,2)); % use only 1 channel for microDoppler
            end
...
%                 disp([int2str(r) '/' int2str(size(RDC,1))]);
                ...
                for proj = 1:length(ang)
                        RDCs{proj}(r,:) = y{proj}(1,:)*((sums(proj)/max_sum)^weight);
                end
        end
...
%                         
%                         for proj = 1:length(ang)
%                                 RDCs{proj}(r,c,:) = y{proj}*((sums(proj)/max_sum)^weight);
%                         end
%                 end
%         end
        
        for i = 1:length(ang)
                sx = myspecgramnew(sum(RDCs{i}(rBin,:,1)),window,nfft,shift); 
                sx2 = abs(flipud(fftshift(sx,1)));
                figure('visible','off');
                colormap(jet(256));
                imagesc(timeAxis,[-prf/2 prf/2],20*log10(sx2 / max(sx2(:))));
